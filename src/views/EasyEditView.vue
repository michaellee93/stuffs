			...
			<section id="add" class="column is-10 content">
				...
				<div class="field">
					...
					<div class="select is-fullwidth">
						<select :disabled="document.version > 1" v-model="content_type" id="type is-primary">
				...
				<div class="field" :id="field.name" :key="i" v-for="(field, i) in currentSchema.fields">
					<label class="label">{{ field.name }}</label>

					<input v-if="field.type === 'text'" v-model="forms[content_type][field.name]" class="input" id="title" />
					...
					<div v-else-if="field.type === 'multiselect'">
						<multi-select :selected.sync="forms[content_type][field.name]" :values="field.values" />
					</div>
					...
					<div v-else-if="field.type == 'data'">
						<table class="table">
							...
							<tbody>
								<tr :key="j" v-for="(block, j) in forms[content_type][field.name]">
									<td>{{ alphatise(j + 26 ** (i - 1)) }}</td>
									<td>
										<new-editor :num="i" :document_id="document_id" :definitions="titles"
											:content.sync="forms[content_type][field.name][j]" :editor-text.sync="editorText"
											:active="j == currBlock && field.name == currRes" @activated="
												currBlock = j;
						...
						<button @click="addBlock(field.name)" class="button is-fullwidth is-info add-block-button">
							Add {{ field.name }}
						</button>
					</div>
